...
用法: python ashare_backtest/ashare_cycle_system.py
"""
import os
import sys
import datetime
...
def main():
    ...
    L.append("- 绝对资金流目前为当日截面(push2his 历史源本环境被限)；系统每日运行会把当日净流入写入 "
             "`ashare_backtest/cache/etf_flow_accum.json`，日积月累自动生成历史累计序列。")
    L.append("- 所有结论为**系统读数与观点**，非交易指令。报告写给未来的自己回看。\n")
    L.append("---")
    L.append("模块: `ashare_backtest/{analog_core, cycle_system, etf_fund_flow, capital_rotation, sector_universe, ashare_cycle_system}.py`")
