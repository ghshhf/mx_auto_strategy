...
用法:
  python3 ashare_backtest/refresh_data.py            # 刷新 data/ashare/bars 下所有已缓存标的
  python3 ashare_backtest/refresh_data.py --flow     # 额外刷新 ETF 资金流累积(需 AkShare 可用)
  python3 ashare_backtest/refresh_data.py --push     # 刷新后顺便 git add + commit + push 到 GitHub

说明:
  - 实时源在沙箱常被封, 本脚本应在「能联网的机器」(如你本机) 运行,
    拉完直接 `git add ashare_backtest/data/ashare && git commit && git push`,
...
def git_push(commit_msg):
    ...
    subprocess.run(["git", "add", "ashare_backtest/data/ashare"], cwd=root)
