...
def main():
    ...
    L.append("- 绝对资金流今日为**当日全市场截面**（主源 AkShare `fund_etf_spot_em`；东财历史资金流 push2his 在本环境被限）。"
             "本脚本每次运行会把当日「最新份额 / 主力净流入」写入 `ashare_backtest/data/ashare/flow/etf_flow_accum.json`(随仓库提交到 GitHub)，"
    ...
    out_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "records")
