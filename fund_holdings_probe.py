...
# 申万一级行业(2021版)标准集合, 用于从妙想多 dto 返回里识别"真实行业字段"
_SW_INDUSTRIES = {
    "农林牧渔", "基础化工", "钢铁", "有色金属", "电子", "汽车", "家用电器",
    "食品饮料", "纺织服饰", "轻工制造", "医药生物", "公用事业", "交通运输",
    "房地产", "商贸零售", "社会服务", "综合", "建筑材料", "建筑装饰",
    "电力设备", "国防军工", "计算机", "传媒", "通信", "银行", "非银金融",
    "石油石化", "煤炭", "机械设备", "环保", "美容护理",
}
# 旧版/别称 -> 标准名(妙想数据新旧命名混用, 统一便于聚合)
_SW_ALIAS = {
    "电气设备": "电力设备", "商业贸易": "商贸零售", "化工": "基础化工",
    "采掘": "石油石化", "休闲服务": "社会服务",
}
...
def _parse_industry(j):
    ...
    妙想行业查询常返回多个 dto: 一个是时间序列(列值形如'商贸零售-一般零售-百货',
    headName=日期), 另一个是单值分类(headName=[' '], 值='通信(申万)')。
    若盲取第一个 dto 的奇数列会被时间序列串味, 故按优先级挑真实行业字段:
      单值分类dto(headName空白) > 值域命中申万标准集 > 无层级串。
    """
    dtos = _dtos(j)
    if not dtos:
        return None
    best = None  # (priority, norm)
    for dto in dtos:
        if not isinstance(dto, dict):
            continue
        t = dto.get("table") or {}
        hn = t.get("headName") or []
        head_blank = (len(hn) == 1 and str(hn[0]).strip() == "")
        for k, v in t.items():
            if k == "headName":
                continue
            ...
            if not val or val in ("-", "None", "null"):
                continue
            norm = _norm_industry(val)
            if not norm:
                continue
            prio = 0
            if head_blank:
                prio += 4
            if norm in _SW_INDUSTRIES:
                prio += 2
            if "-" not in val:
                prio += 1
...
def _norm_industry(ind):
    """归一为申万一级: 去括号注释('通信(申万)'->'通信'), 层级串取第一段,
    再走别名表('电气设备'->'电力设备')。"""
    if not ind:
        return ind
    ind = ind.strip()
