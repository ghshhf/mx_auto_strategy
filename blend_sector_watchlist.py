...
def _html(df, rows, btc_m, pairs):
    ...
    rows_html = "".join(
        f"<tr><td>{r['zh']}</td><td>{r['code']}</td><td class='c'>{r['btc_corr']:+.3f}</td>"
        f"<td class='r'>{r['multiple']:.2f}x</td><td>{r['cagr']:.1f}%</td>"
        f"<td class='r'>{r['mdd']:.1f}%</td><td>{r['vol']:.1f}%</td><td>{r['sharpe']:.2f}</td></tr>"
        for r in rows)
    minp = "".join(f"<li>{a}-{b}: <b>{c:.3f}</b></li>" for c, a, b in pairs[:6])
    maxp = "".join(f"<li>{a}-{b}: <b>{c:.3f}</b></li>" for c, a, b in pairs[-3:])
    return f"""<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8">
<title>美股行业指数 vs BTC 相关性 & 再平衡观察清单</title>
...
h1{{font-size:23px;margin:0 0 4px;}} .sub{{color:#9aa3b2;margin-bottom:18px;}}
.card{{background:#171a23;border:1px solid #262b38;border-radius:14px;padding:20px;margin-bottom:20px;}}
table{{border-collapse:collapse;width:100%;font-size:13.5px;}} th,td{{border-bottom:1px solid #2a3040;padding:7px 9px;text-align:left;}}
td.r{{text-align:right;font-variant-numeric:tabular-nums;color:#ffd479;}} td.c{{text-align:right;color:#7ee787;}}
.note{{color:#9aa3b2;font-size:13px;line-height:1.75;}} .hl{{color:#7ee787;}} .warm{{color:#ffb86c;}}</style></head>
<body>
<h1>美股行业指数 × BTC 相关性 & 再平衡观察清单</h1>
<div class="sub">11 个 SPDR 行业ETF vs 比特币 · 共同窗口 2018-06-22~2026-08-28 (~8年) · 方法论证非业绩承诺</div>

<div class="card"><h3 style="margin-top:0">各行业 vs BTC 相关系数（升序：越低越能为加密账户减震）</h3>
<table><tr><th>行业</th><th>代码</th><th>BTC相关</th><th>倍数</th><th>CAGR</th><th>MDD</th><th>年化波动</th><th>Sharpe</th></tr>
{rows_html}</table>
<p class="note">BTC 单资产(同窗): {btc_m['multiple']:.1f}x / CAGR {btc_m['cagr']*100:.0f}% / MDD {btc_m['mdd']*100:.0f}% / 年化波动 { (df.pct_change().fillna(0)['BTC'].std()*np.sqrt(52)*100):.0f}%。<br>
<b>核心结论：</b>11 个行业与 BTC 相关系数全部落在 <span class="hl">−0.03 ~ +0.11</span>，近乎零相关 —— 对你的加密重仓账户，任何行业指数都是强分散项，加行业指数 = 给你的 −75% 加密回撤装减震器。</p></div>

<div class="card"><h3 style="margin-top:0">行业间相关性（配多个行业时用于波动收益）</h3>
<p class="note"><b>最低相关配对（适合组合内互相再平衡）：</b><ul style="margin:4px 0">{minp}</ul>
<b>最高相关配对（避免重复配置）：</b><ul style="margin:4px 0">{maxp}</ul>
能源(XLE) 与几乎所有行业相关都低(0.30~0.40)，是行业内的"异类"，最适合作波动收益引擎；科技-可选消费-通信属成长集群(0.8+)宜二选一。</p></div>

<div class="card"><h3 style="margin-top:0">净值曲线（对数轴, 起点=1）</h3>
<div id="c1" style="width:100%;height:480px"></div>
<p class="note">可见：科技(XLK)与 BTC 同窗都涨最多但路径迥异；医疗/必需消费/公用事业最平稳，是压舱石候选。</p></div>

<script>
const D = {{dates:dates, series:{series}}};
const TR = ['BTC','XLK','XLV','XLP','XLU','XLE','XLI','XLF','XLB','XLY','XLRE','XLC'].filter(k=>k in D.series);
Plotly.newPlot('c1', TR.map(k=>({{x:D.dates,y:D.series[k],name:k,mode:'lines'}})),
