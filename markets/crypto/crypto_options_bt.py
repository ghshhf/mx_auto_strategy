...
class CryptoOptionsConfig:
    ...
    short_proactive_cooldown: int = 10       # 主动做空信号冷却（2026-08-13优化: 10优于13, 10y +14%/5y +17%/3y MDD改善; 更灵活的做空节奏）
...
def run_bt(px, cfg_dict=None, label='V6_options', start=None,
    ...
    if cycle_overlay:
        try:
            sys.path.insert(0, os.path.dirname(os.path.dirname(HERE)))  # 仓库根(cycles 包所在)
