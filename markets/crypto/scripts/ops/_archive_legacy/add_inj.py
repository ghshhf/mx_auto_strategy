...
if _CS_ROOT != _SCRIPT_DIR:
    ...
    _os.chdir(_CS_ROOT)
# --- [relocated] 引导结束 ---
"""抓取 INJ(Injective) 周K线并入 3 个面板 CSV.
...
def main():
    weekly = fetch_inj_weekly()
    if not weekly:
        raise SystemExit("INJ 拉取为空, 终止")
    ks = sorted(weekly)
    print(f"[+] INJ Binance周K: {len(weekly)} 周, {ks[0]} -> {ks[-1]}, 首值={weekly[ks[0]]:.4f}")
    for f in FILES:
        df = pd.read_csv(f, index_col=0, parse_dates=True)
        inj = pd.Series(index=df.index, dtype=float)
        for i, d in enumerate(df.index):
            key = d.strftime("%Y-%m-%d")
            if key in weekly:
                inj.iloc[i] = weekly[key]
        if "INJ" in df.columns:
            df = df.drop(columns=["INJ"])
        df["INJ"] = inj.values
        fv = df["INJ"].first_valid_index()
        print(f"{f}: 加 INJ 列, 共 {df.shape[1]} 列, 首次有效={fv.date() if fv else None}")
