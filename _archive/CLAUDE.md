...
> ⚠️ **项目现状（2026-08）**：龙虾炒股大赛 **live 自动交易方向已搁置**。本仓库当前定位为 **A股 / 美股 / 加密三市场量化回测研究引擎**（非实盘执行系统）；权威真值见 [`docs/TRUTH.md`](docs/TRUTH.md)。`auto_trader.py` / `grid_trader.py` / `sync_contest.py` 等为 **legacy** 代码，保留参考，不再主维护。
