...
| 市场 | 引擎目录 | 权威真值 | 说明 |
|------|----------|----------|------|
| A股 | `markets/ashare/` | 头条 18.185x · CAGR 22.31% · MDD −33.31% | 腾讯后复权周线 + momentum26 + 核心卫星 0.5 + 死叉 |
| 美股 | `markets/us/` | 期权模拟 **106.19x** · 无期权 **21.95x**（2026-09-10 按 `nav_us.json` 刷新；旧 99.85x / 22.48x 已漂移） | 真实面板 + 公允 BS 期权模拟层（非稳健真值，仅供对照）；期权层非杠杆收租/保险 |
| 加密 | `markets/crypto/` | 周期引擎 7,637.77x (发布 NAV) · 减半相位叠加 | 含幸存者偏差；减半周期相位减仓为合法 alpha；**期权三件套已于 2026-08-31 关闭** |
