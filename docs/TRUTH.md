...
### 美股（`markets/us/`）
- **头条 106.19x** · CAGR 47.2% · MDD −48.3%（期权增强，真实面板 + 公允 BS 期权定价；**2026-09-10 按 `nav_us.json` 产物刷新**，旧标 99.85x / −47.2% 已漂移作废）
- 无期权基线 **21.95x**（旧标 22.48x 已作废）；保守下限 31.7x
...
- **头条 5,630.11x** · CAGR 135.2% · MDD −70.4% · Sharpe 1.60（**27 币池** · 现货轮动 + 减半相位叠加 = 发布 NAV，2026-09-10 重跑）
  - 🔄 **2026-09-08 池变更重跑（当前）**：32 币 → **27 币**（终版池，见 `_screen_current_pool.json` 的 `panel=v3(27)`）。原 32 币头条 **5,662.51x 已作废**
  - 🔄 2026-09-06 池变更：删 ICP（无硬顶供应，Mission 70 前通胀 9.72%）+ INJ（软顶 1 亿但动态增发 5%–10%，机制过复杂）→ 34 币降为 32 币
  - ⚠️ 34 币时代的 **7,637.77x / 21,419x / 8,488x / 26,211x 全部作废**，不可再引用
- 口径变体（**27 币池，2026-09-08 重跑**）：reconcile 10y FULL（inv_vol + 1.2x + 周期）**6,059x** / −70.5% / Sharpe 1.60 / CAGR 136.9%
  - ⚠️ reconcile 的 FULL 口径与上方 cycle_overlay 头条**不是同一算法**，二者不可互相替换引用
- 数据：`weekly_adjclose_crypto50_10y.csv` 为唯一真值基准（**27 币**，末日 2026-09-04，Binance/OKX/Gate）；c50/v3/10y 三面板币种完全一致、零偏差零空洞
  - 📌 文件名里的 "crypto50" 是历史遗留（面板曾为 50 币），**实际只有 27 列**，勿按文件名推断币数
...
| 加密（单市场，周期引擎） | 532.39x | 100.3% | −70.4% | 1.38 |
| 等权（1/3） | 143.12x | 73.2% | −38.6% | 1.98 |
| 波动平价（逆波动） | 46.89x | 53.1% | −32.9% | 1.98 |
| 稳健倾斜（.4/.4/.2 加密） | 89.46x | 64.4% | −31.4% | 1.93 |
| **波动平价（季再平衡·封顶 60%）** | 46.97x | 53.1% | −32.3% | 1.95 |

> 注（2026-09-10 按 27 币面板重算）：上表为**三市场共同窗口**口径。加密单市场 532.39x 是截到共同窗口的结果；其独立 10y 全窗口真值为 **5,630.11x**（见上节头条）。A股/美股头条（18.185x / 106.19x）为各自全窗口口径，与共同窗口数值不同属正常。
> ⚠️ 旧表曾写「加密独立真值 7,637.77x」—— 那是 34 币池数字，本章已作废，勿再引用。
