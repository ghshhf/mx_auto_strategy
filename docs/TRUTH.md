...
期权层 = 远期（≥1 年 DTE）收租 / 保险，**非杠杆**，权利金几个百分点 / 年 ≈ 费用级（**当前仅美股保留期权层；加密期权层已于 2026-08-31 关闭**）。
...
### 加密（`markets/crypto/`）
- 🛑 **期权三件套已于 2026-08-31 关闭**：审计发现 put 保险层被误建模为收益引擎，关闭后逐腿拆解倍率变化恒为 Δ=+0.0%。**448.6x / 24,494x / 37,815x / 59.36Mx 等均为期权时代数字，已废弃，不可作现真值引用**
- **头条 7,637.77x** · CAGR 142.8% · MDD −69.6% · Sharpe 1.64（34 币池 · 现货轮动 + 减半相位叠加 = 发布 NAV）
- 口径变体：减半相位叠加关（base）21,419x；reconcile 10y FULL（inv_vol + 1.2x + 周期）8,488x / −69.6% / Sharpe 1.65
- 数据：`weekly_adjclose_crypto50_10y.csv` 为唯一真值基准（34 币，末日 2026-08-28，Binance/OKX/Gate）；c50/v3 已对齐、三面板零偏差零空洞
...
- 引擎 `markets/crypto/backtest_v2.py`（现货轮动，在用）+ 导出 `markets/crypto/export_nav_crypto.py`；~~`crypto_options_bt.py` 期权引擎已停用~~；学习页 `markets/crypto/reports/crypto_17000_explainer.html`；机制文档 `docs/CYCLE_DERISK.md`
- 📌 下方 ⚠️ 两条为**减半相位方法论**的实证记录（相位叠加至今仍在用，是当前 NAV 的组成），但其中引用的倍数（24,494x / 37,815x / 18,378x）**产自期权时代面板，仅作方法论证据，非现真值**
...
## 跨市场组合（`portfolio_blend.py`，共同窗口 2017–2026）

| 方案 | 倍数 | CAGR | MDD | Sharpe |
|---|---|---|---|---|
| A股（单市场） | 6.83x | 23.7% | −21.3% | 1.19 |
| 美股（单市场，期权模拟） | 118.13x | 69.5% | −48.3% | 1.24 |
| 加密（单市场，周期引擎） | 736.22x | 107.6% | −69.6% | 1.43 |
| 等权（1/3） | 159.82x | 75.3% | −38.6% | 2.01 |
| 波动平价（逆波动） | 50.41x | 54.3% | −32.9% | 2.00 |
| 稳健倾斜（.4/.4/.2 加密） | 95.62x | 65.6% | −31.3% | 1.95 |
| **波动平价（季再平衡·封顶 60%）** | 50.39x | 54.3% | −32.3% | 1.98 |

> 注：上表为**三市场共同窗口**口径。加密单市场 736.22x 是截到共同窗口的结果；其独立 10y 全窗口真值为 **7,637.77x**（见上节头条）。A股/美股头条（18.185x / 99.85x）为各自全窗口口径，与共同窗口数值不同属正常。

核心论点：三市场低相关，分散在不牺牲太多倍数的前提下显著压低 MDD、抬升 Sharpe。
「季再平衡·封顶 60%」为真正可执行分配器：每 13 周按回看波动重算逆波动目标权重（单市场 ≤60%），区间内含息持有。

## 期权哲学（贯穿三引擎）
- 选 ≥1 年 DTE 远期期权；权利金 = 名义的几个百分点 / 年（美股 call 4.5% / put 6%；加密因 IV 高 call 13–28% / put 15.6% —— **加密层已关闭，此为历史口径**）
- 卖 call = 收租（净现金流入 ≈0）；买 put = 保险（成本封顶）→ **非杠杆暴露**
- 只有「买 call 作股票替代（deep ITM，名义 10–30%）」才是真杠杆，三引擎均不做

## 诚实口径
- 加密含幸存者偏差（现存主流币清单）；美股期权层为公允定价下可辩护值（**加密期权层已于 2026-08-31 关闭，现真值不含任何期权贡献**）
