...
- **加密**：OOS 显著增益倍数（+22.3%, t=5.4）但 **MDD 显著恶化（t=−5.98，约 +6pp，下文统一约定：MDD 变化 正=恶化）**。**in-sample 的 MDD 改善是 2022 单事件幻觉**（与项目估值层同性质：全样本被一次崩盘压平，walk-forward 1y 窗口下效应消失）。叠加层实为**顺周期收益放大器，非保险**。

**加密机制扫描（optimize_crypto_asym.py，对称 / 下行保护型 / 纯保险型三类）：**
- **没有任何机制能消除回撤恶化**：三类在所有窗口 MDD 变化均显著为正（in-sample +4~+11pp；OOS 对称 tilt=0.5 的 t(MDD)=+6.4）。
- **纯保险型（顺风不加仓, up=0）虽减回撤，却牺牲全部收益**：10y 倍数比 < 1.0（负增益）—— 加密增益主要来自「宏观宽松期顺趋势加仓」，而非「逆风减仓躲崩盘」。
- **根因**：这 4 个慢变量宏观周期（fed_rate/liquidity/commodity/housing）对加密是**顺周期动量确认**，加密自身崩盘幅度（−80%）远超其减仓幅度（下限砍半到 0.5），减仓根本躲不过崩盘。

**加密 tilt 权衡（OOS，符号约定：MDD 变化 正=恶化）：**

| tilt | 几何倍数比 | 均值 MDD 变化(正=恶化) |
|------|-----------|----------------------|
| 0.2 | +10.8% | +2.7pp |
| 0.3 | +15.9% | +4.1pp |
| 0.4 | +19.5% | +5.3pp |
...
→ 任何力度下都是「更多收益 + 更多回撤」的单调权衡，无保险型有效。默认 tilt=0.3 = **克制放大器档位**（仍显著 +16% 倍数，回撤代价较轻）；要更高收益可提 0.5；要回撤控制则 `cycle_overlay=False`（回到无叠加基线）。加密叠加层是**收益/回撤的 trade-off 旋钮，不是风控装置**。
