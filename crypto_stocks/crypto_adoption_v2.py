...
THEME_COINS = {
    ...
    "DeFi":    ['UNI', 'LINK', 'AAVE', 'MKR', 'SNX', 'COMP', 'CRV', 'DYDX', '1INCH', 'ENS', 'LDO'],
    "DeFi借贷": ['AAVE', 'COMP', 'CRV'],  # 与 DeFi 重叠, 复用
    "DEX":     ['UNI', 'CRV', '1INCH', 'DYDX', 'JUP'],
...
def offense_weights_for_year(year, valid=None, mode='theme_first'):
    """
    ...
    if mode == 'theme_first':
        ...
        for theme, coins in THEME_COINS.items():
            ...
            m = ad["multiplier"]
            if m <= 0: continue
            avail = [c for c in coins if (not valid or c in valid)]
...
def offense_top_n(year, n=5, valid=None, px=None, as_of=None):
    ...
    if px is not None and len(px) > 52:
        ...
        if as_of is not None:
            try: idx = px.index.get_loc(as_of)
            except: idx = len(px) - 1
        else:
