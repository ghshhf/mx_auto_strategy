...
|---|---|---|---|---|
| baseline（v6.14b 修正：弱市停车进**分红防御篮**、去黄金、季频、等权 Top10） | 5.87x | 15.9% | −29.5% | 3.80x |
| **optimized**（美股优化引擎，无杠杆，见 7.2） | **29.91x** | 32.7% | −47.8% | 3.80x |
| **optimized + AI 选股层**（确定性质量乘数加权） | **30.41x** | 32.9% | −46.9% | 3.80x |
| **optimized + 1.2x 净杠杆**（冲 40~50x 的合法路径） | **44.36x** | 37.2% | −55.9% | 3.80x |

- **无杠杆硬上限 ≈30x**，恰好 ≈ A 股同方法论 16.29x 的 **两倍**（"美股≈A股两倍"直觉成立：美股长牛、赢家更集中）。
- **40~50x 需要适度净杠杆**：1.2x 总敞口 → 44.4x（落入目标区间）；1.3x → 53x（已超 50）。杠杆放大回撤（44x 档 MDD −56%），且**借入成本未计**（实盘约 5%/年会小幅削减），属标准机构做法而非过拟合。
- AI 选股层净效应：无杠杆 +1.7%（29.91x→30.41x）、MDD 改善 +0.9pp。
- 关键年份（optimized）：2020 +44.2% / 2023 +177.3% / 2024 +457.7% 精准抓住 AI 爆发；2018 −16.7%、2022 −23.4% 为可控回撤。

### 7.2 优化引擎设计（对齐 A 股 16 倍方法论，适配美股特征）

**防御端（用户指定，v6.15 修正）：彻底去黄金。**
- 防御 = **分红股**（动态选近 20 周低波动候选 KO/ABBV/JJN/PG/XOM/VZ/MO 等，面板已有真实数据）+ **债券**（极端防御/crash 时若有债券 ETF 列 TLT/IEF/AGG/BND，则把现金仓改配债券作危机对冲）。
- 防御占比**整体小**：平衡 5% / 弱市 15% / crash 15%，符合"防御配置不大"的要求。
- 债券数据当前面板**尚未并入**（沙箱无法自动抓取）；运行 `extend_panel_bonds.py`（同款 westock-data 机制，先抓 raw_tlt/ief/agg/bnd.txt）即可让 crash 档自动启用债券对冲。未并入时 crash 退化为现金。
...
```bash
cd us_stocks
python3 us_backtest_ai.py                       # 四档全跑(确定性 AI, 杠杆档默认1.2x)
...
python3 us_backtest_ai.py --no-ai               # 关闭 AI(仅 baseline+optimized+杠杆)
python3 us_backtest_ai.py --lev 1.3             # 杠杆档用 1.3x(→~53x, 回撤放大)
# 输出 us_stocks/data/us_nav_ai.csv (date, baseline_nav, optimized_nav, [optimized_ai_nav], optimized_lev_nav)
```

### 7.5 关于「40~50x」的诚实说明

- **无杠杆**：真实宇宙 + 无前视 + 动态池的硬上限就是 **~30x**（≈2× A 股 16x）。扫参 162 组（Top N×再平衡周期×趋势门×动量窗口×crash 现金）全部停在 30x 附近，无法靠调参突破。
- **冲 40~50x 的合法路径 = 适度净杠杆**：1.2x → 44x、1.3x → 53x。这是标准机构做法（许多系统化股票基金跑 1.2~1.5x 总敞口），非幸存者偏差。
- **不可取路径**（刻意不做）：① Top1 单一名 → 倍数更低且 MDD −78%（单笔错配近乎归零）；② 后见之明精选 NVDA（已知它会涨）或只留赢家（幸存者偏差）——均属过拟合/前视。

结论：**无杠杆 30x 是该真实宇宙下最优可辩护结果（已超 A 股 16x 近一倍、远超 SPY 3.8x）；要达你预期的 40~50x，启用 1.2~1.3x 净杠杆即可，回撤同步放大至 −56~−60%。**
