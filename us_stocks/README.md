...
| **optimized**（美股优化引擎，无杠杆，主题解相关 max2，见 7.2） | **33.69x** | 34.1% | −46.0% | 3.80x |
| **optimized + AI 选股层**（确定性质量乘数加权） | **34.42x** | 34.3% | −45.6% | 3.80x |
| **optimized 防御档**（默认 20% 现金袖，见 7.2） | **20.08x** | 28.4% | −38.6% | 3.80x |
| **optimized + 1.2x 净杠杆**（可选，默认关闭，见 7.5） | **51.60x** | 38.9% | −53.3% | 3.80x |

- **无杠杆 ≈ 34x**（主题解相关 max2 修复回归后从虚低的 29.91x 回到真值 33.69x），恰好 ≈ A 股同方法论 16.29x 的 **两倍**（"美股≈A股两倍"直觉成立：美股长牛、赢家更集中）。**已超用户 30x 目标**。
- **AI 选股层净效应**：无杠杆 +2.2%（33.69x→34.42x）、MDD 改善 +0.4pp。
- **MDD 是结构性问题**：纯动量高 beta 满仓，每一年（2018–2026）都有 −24%~−47% 年内回撤，2022 最深 −46.3%。**事后信号（MA/护栏/止损/广度）均压不住**（要么太迟要么鞭梢）；唯一有效手段是**结构性降敞口**（现金袖 / 波动率目标），代价是倍数。
- 关键年份（optimized）：2020 +43.4% / 2023 +158.3% / 2024 +429.8% 精准抓住 AI 爆发；2018 −15.9%、2022 −23.9% 为可控回撤（但全年仍深）。

#### MDD 压缩前沿（结构性现金袖 `--struct-def`，无杠杆，无 vol 目标）

| 现金袖 | 倍数 | CAGR | MDD |
|---|---|---|---|
| 0%（满仓 optimized） | 33.69x | 34.1% | −46.0% |
| 15% | 23.05x | 29.9% | −40.5% |
| 20%（默认防御档） | 20.08x | 28.4% | −38.6% |
| 40% | 10.94x | 22.1% | −30.5% |
| 40% + volT0.22 | 10.05x | 21.2% | −28.3% |

> **用户目标"30x + MDD≈−30%"不可兼得（无杠杆下）**：34x 对应结构性 −46%；要压到 −30% 必须把现金袖加到 40%，倍数同步掉到 ~11x。这是真实权衡曲线，不是参数没调好。杠杆（7.5）可在保留倍数的同时放大回撤，但同样不压 MDD。
...
python3 us_backtest_ai.py --struct-def 0.40     # 防御档切 40% 现金袖(→~11x / MDD≈−30%)
python3 us_backtest_ai.py --struct-def 0.40 --vol-target 0.22  # 袖+波动目标(→~10x / MDD≈−28%)
...
python3 us_backtest_ai.py --lev 1.3             # 杠杆档用 1.3x(→~62x, 回撤放大, 默认1.0关闭)
...
- **无杠杆 ≈ 34x（已超 30x 目标）**：真实宇宙 + 无前视 + 动态池 + 主题解相关的硬上限就是 **~34x**（≈2× A 股 16.29x）。主题解相关 max2 修复回归后从虚低的 29.91x 回到真值 33.69x。扫参 162 组全部停在 30x+ 附近，无法靠调参突破。
- **但 34x 的 MDD 是结构性 −46%**，集中在纯动量高 beta 满仓。要压到用户预期的 **MDD≈−30%**，必须上 40% 现金袖，倍数同步掉到 **~11x**（见 7.1 前沿表）——**"34x 且 −30%" 在无杠杆下不可兼得**，这是权衡曲线不是调参遗漏。
- **冲 40~50x 的合法路径 = 适度净杠杆（用户已选不用）**：1.2x → 51.6x、1.3x → 61.7x。这是标准机构做法（许多系统化股票基金跑 1.2~1.5x 总敞口），非幸存者偏差；杠杆**放大回撤**（1.2x 档 MDD −53%）且借入成本未计。
...
结论：**无杠杆 34x（MDD −46%）是该真实宇宙下最优可辩护的进攻结果（≈2× A 股、远超 SPY 3.8x）；若坚持 MDD 压到 −30% 左右，走 40% 现金袖 → ~11x 是诚实答案；要达你早先预期的 40~50x，启用 1.2~1.3x 净杠杆即可。三者是同一引擎上的不同风险档，由 `--struct-def` / `--lev` 旋钮选择。**
