...
| **加密 10y** | 周期引擎 **5,662.51x** / MDD −70.4% / Sharpe 1.60（cycle_overlay 发布口径；reconcile 10y FULL 6,030x / 12y 18,622x） | 525 周（2016-08-11 起），**32 币**（2026-09-06 删 ICP/INJ，原 34 币 7,637.77x 作废）；同期 BTC 买入持有 ≈15.9x。**期权三件套已于 2026-08-31 关闭**，头条为现货轮动+减半相位叠加。完整真值表见 `TRUTH_AUTHORITY.md` |

> ⚠️ **旧口径均已废弃**：`59.36Mx`（期权三件套时代，2026-08-31 已关闭）、`28,092x`（43 币+期权时代）、`24,493x`（2026-08-11 前 DEFAULT_CFG）均不可作为现真值引用。当前真值 = 现货轮动+减半相位叠加（**5,662.51x** 发布 NAV / **6,030x** reconcile 10y FULL，32 币池，2026-09-06 重跑）。
