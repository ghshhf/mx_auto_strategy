...
| **加密 10y** | 周期引擎 **5,630.11x** / MDD −70.4% / Sharpe 1.60（cycle_overlay 发布口径；reconcile 10y FULL 6,059x / 12y 18,711x） | 526 周（2016-08-12 起），**27 币**（2026-09-08 终版池；2026-09-06 删 ICP/INJ 34→32，原 34 币 7,637.77x 作废）；同期 BTC 买入持有 ≈15.9x。**期权三件套已于 2026-08-31 关闭**，头条为现货轮动+减半相位叠加。完整真值表见 `TRUTH_AUTHORITY.md` |

> ⚠️ **旧口径均已废弃**：`59.36Mx`（期权三件套时代，2026-08-31 已关闭）、`28,092x`（43 币+期权时代）、`24,493x`（2026-08-11 前 DEFAULT_CFG）均不可作为现真值引用。当前真值 = 现货轮动+减半相位叠加（**5,630.11x** 发布 NAV / **6,059x** reconcile 10y FULL，27 币池，2026-09-10 重跑）。
...
- **`held_weeks.json` 不是持仓**：它是回测统计（权重>0 周数），顶层带 `_meta` 声明。个人持仓无文件，以用户口述为准。增删币后跑 `python markets/crypto/refresh_stats.py` 重算四件派生 JSON（`manage_token.py add/remove` 收尾会自动跑，可用 `--no-refresh` 跳过）。
