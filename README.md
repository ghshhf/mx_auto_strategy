...
| **加密 10y** | 周期引擎 **7,637.77x** / MDD −69.6% / Sharpe 1.64（cycle_overlay 发布口径；reconcile 10y FULL 8,488x / 12y 26,211x） | 525 周（2016-08-11 起），**34 币**；同期 BTC 买入持有 ≈15.9x。**期权三件套已于 2026-08-31 关闭**，头条为现货轮动+减半相位叠加。完整真值表见 `TRUTH_AUTHORITY.md` |

> ⚠️ **旧口径均已废弃**：`59.36Mx`（期权三件套时代，2026-08-31 已关闭）、`28,092x`（43 币+期权时代）、`24,493x`（2026-08-11 前 DEFAULT_CFG）均不可作为现真值引用。当前真值 = 现货轮动+减半相位叠加（7,637.77x 发布 NAV / 8,488x reconcile 10y FULL）。
